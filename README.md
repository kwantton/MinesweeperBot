...
**Note!** This version does not clone the mine layout generator of the original minesweeper; the very original ones up until 2007 _first_ generate the map, then move the mine to the 1st row in case the user would have hit it on the 1st click (so the 1st click is guaranteed to be safe by replacing the mine to the first vacant cell on the map). Windows Vista onwards (2007), the map is generated _after_ the first click, and that's the way my `Minesweeper()` works as well. Here all the mines are placed after evenly **after** the first click.
