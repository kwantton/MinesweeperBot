...
**Note!** This version does not clone the mine layout generator of the original minesweeper; the very original ones up until 2007 first generates the map, then moves the mine to the 1st row in case the user has hit it. Windows Vista onwards (2007), the map is generated _after_ the first click, and that's the way my `Minesweeper()` works as well. Here the all the mines are placed after evenly **after** the first click.
