...
class CSP_solver:
    ...
    def absolut_brut(self) -> None:
        ...
        def join_comp_groups_into_solutions(compatibility_groups:dict) -> None:
            ...
            def traverse(proposed_origin, proposed_matches_for_the_key, entered_alts_for_this_build, 
                possible_solution_build, already_handled_groups): # I'm keeping 'proposed_origin' solely for debugging purposes!
                
                # NB! The traverse from each must happen BEFORE adding to solved or seen values; otherwise the handling of the alts will never occur from one spot.
                add_these_to_handled_groups_after_iterating_through_every_single_proposed_matching_alt = []
                add_these_to_entered_alts_after_iterating_through_every_single_proposed_matching_alt = []
                for proposed_matching_alt in proposed_matches_for_the_key:
                    add_these_to_entered_alts_after_iterating_through_every_single_proposed_matching_alt.append(proposed_matching_alt)
                    # entered_alts_for_this_build.add(proposed_matching_alt)              # incompatible or not, I should never re-enter an alt solution later again; just mark it as seen, and that's it.
                    if proposed_matching_alt in compatibility_groups:                   # If this alt solution is not a key in 'compatibility_groups', then IT IS UNTRUE; then do NOT handle this alt (do not (1) mark its variables' proposed values as possible solutions, and do not (2) mark the group that it presents as handled); if the current alt is NOT present as a key of 'compatibility_groups', IT IS INCOMPATIBLE WITH AT LEAST ONE OTHER GROUP. In English, if the current alt solution is not present as a key in 'compatibility_groups', it CANNOT EVER SATISFY ALL THE EQUATIONS that we know MUST be true using at least one combination of alt solutions.
                        ...
                        if group_of_this_alt not in already_handled_groups:             # NB! So, I only want EXACTLY ONE alt solution per group. If an alt solution has already been handled, DO NOT HANDLE ANOTHER; that another alt solution will be handled in a whole another iteration of this 'traverse()'. (2) Comparison of sets in python; if the values in the set are the same, then the sets are 'equal' in the == comparison. Nice.
                            incompatible_pma = check_for_disagreements(
                            ...
                            add_these_to_handled_groups_after_iterating_through_every_single_proposed_matching_alt.append(group_of_this_alt)    # NB! If I added them here, you know what's the problem? Iteration through 'proposed_matches_for_the_key' is still unfinished! For the yet-unexplored alternative alts to the current one, if one of them is in the same group as the already-handled group, it would not be processed correctly in the iteration because of my checks regarding group being handled already!
                            
                            already_handled_groups_updated = already_handled_groups.copy()      # for the traverse() iteration below, I need to create a copy of the current state AND add the just-handled group into that list!
                            ...
                            # SO: for every set of new_matches, we want the info that was updated according to what happened in the specific alternative above; which alt solution ('proposed_matching_alt') was 'entered' (seen, processed), which 'group' (equation) in question was handled. Since all the alternatives in the above loop are indeed ALTERNATIVES, they are NOT saved immediately, that is done after the 'traverse()'s below!
                            if new_matches:
                                for new_match in new_matches:
                                    if new_match not in entered_alts_for_this_build:    # unnecessary double-check, to-do: remove after you get everything working.
                                        traverse(proposed_origin, new_matches, 
                                            entered_alts_for_this_build_updated, possible_solution_build, already_handled_groups_updated)
                for group_of_alt in add_these_to_handled_groups_after_iterating_through_every_single_proposed_matching_alt:
                    already_handled_groups.append(group_of_alt)
                for entered_alt in add_these_to_entered_alts_after_iterating_through_every_single_proposed_matching_alt:
                    entered_alts_for_this_build.add(entered_alt)              # incompatible or not, I should never re-enter an alt solution later again; just mark it as seen, and that's it.
                if n_groups == len(already_handled_groups):                 # NB! I could also check if all variables are present in 'solution_build', but this is clearer. It's technically equivalent and applicable here, though.
                    ...
                    return                                                  # technically unneeded 'return', but emphasizes the fact that if all groups have been handled (in English, if one alt solution from each equation has been incorporated in the current 'possible_solution_build', then this has been saved as a possible whole solution, and nothing should be done in this specific 'solution_build' any longer)
