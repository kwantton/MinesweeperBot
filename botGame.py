...
class Minesweeper:
    ...
    def handle_probing_of_already_opened_cell(self, x, y):                      # either it's a chording, or 
        print('- OPEN ALREADY')
        neighbours = self.neighbours_coordinates(x, y)                          # finds all the actual cells neighbouring (x,y)
        n_surrounding_flags = self.count_surrounding_flags(neighbours)
        label = self.map[y][x]
        if label == cell_id(n_surrounding_flags):
